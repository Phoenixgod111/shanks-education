...
class SubjectManager {
    ...
    async loadSubjectContent(grade) {
        ...
        // ╨Ч╨░╨│╤А╤Г╨╢╨░╨╡╨╝ ╨║╨╛╨╜╤В╨╡╨╜╤В ╨┤╨╗╤П ╨║╨░╨╢╨┤╨╛╨│╨╛ ╨┐╤А╨╡╨┤╨╝╨╡╤В╨░ ╨┤╨╛╤Б╤В╤Г╨┐╨╜╨╛╨│╨╛ ╨▓ ╤Н╤В╨╛╨╝ класс╨╡
    ...
    async loadAllSubjectContent() {
        ...
        // ╨Ч╨░╨│╤А╤Г╨╢╨░╨╡╨╝ ╨║╨╛╨╜╤В╨╡╨╜╤В ╨┤╨╗╤П ╨▓╤Б╨╡╤Е ╨┐╤А╨╡╨┤╨╝╨╡╤В╨╛╨▓ ╨╕ ╨▓╤Б╨╡╤Е ╨╕╤Е класс╨╛╨▓
        const loadPromises = [];
        let totalFiles = 0;
    ...
    async loadSubjectTopics(subjectId, grade) {
        try {
            ...
            // Обогащаем темы placeholder-практикой, если practice/lessons/quiz пусты
            if (topicsData.topics && Array.isArray(topicsData.topics)) {
                topicsData.topics = topicsData.topics.map((t, i) => this.enrichTopic(t, subjectId, grade, i));
            }
    ...
    /** Обогащает тему placeholder-практикой, уроками и тестом, если они пусты */
    enrichTopic(topic, subjectId, grade, index) {
        const title = topic.title || `Тема ${index + 1}`;
        const needsPractice = !topic.practice || (Array.isArray(topic.practice) && topic.practice.length === 0) ||
            (typeof topic.practice === 'object' && !topic.practice.easy && !topic.practice.medium && !topic.practice.hard);
        const needsLessons = !topic.lessons || topic.lessons.length === 0;
        const needsQuiz = !topic.quiz || !topic.quiz.questions || topic.quiz.questions.length === 0;

        const enriched = { ...topic };

        if (needsLessons) {
            enriched.lessons = [
                { id: `lesson-${topic.id || index}-1`, title: `Урок 1: ${title}`, type: 'theory', duration: 15, content: `Теоретический материал по теме "${title}".` },
                { id: `lesson-${topic.id || index}-2`, title: `Урок 2: Примеры`, type: 'theory', duration: 20, content: `Разбор примеров по теме "${title}".` }
            ];
        }
        ...
        if (needsPractice) {
            enriched.practice = {
                easy: [
                    { id: `p-easy-${topic.id || index}-1`, title: 'Базовые упражнения', type: 'exercises', count: 5, description: 'Простые задания для закрепления', difficulty: 'easy' }
                ],
                medium: [
                    { id: `p-medium-${topic.id || index}-1`, title: 'Средние задачи', type: 'exercises', count: 8, description: 'Задачи средней сложности', difficulty: 'medium' }
                ],
                hard: [
                    { id: `p-hard-${topic.id || index}-1`, title: 'Сложные задачи', type: 'word-problems', count: 5, description: 'Задачи повышенной сложности', difficulty: 'hard', isPremium: true }
                ]
            };
        }
        ...
        if (needsQuiz) {
            enriched.quiz = {
                id: topic.quiz?.id || `quiz-${topic.id || index}`,
                    ...
                    <div class="topics-list-compact">
                        ${topics && topics.length > 0 ? topics.map((topic, index) => {
                            ...
                            <div class="empty-state">
                                ...
                                <p>╨в╨╡╨╝╤Л ╨┤╨╗╤П ╨┐╤А╨╡╨┤╨╝╨╡╤В╨░ "${subjectInfo?.name || subjectId}" ╨▓ ${grade} класс╨╡ ╨╡╤Й╨╡ ╨╜╨╡ ╨┤╨╛╨▒╨░╨▓╨╗╨╡╨╜╤Л</p>
                            </div>
                        `}
                        ...
                        <div class="theory-content">
                            ${topic.lessons && topic.lessons.length > 0 ?
                                ...
                                <div class="empty-state interactive">
                                    ...
                                    <h3>╨в╨╡╨╛╤А╨╡╤В╨╕╤З╨╡╤Б╨║╨╕╨╣ ╨╝╨░╤В╨╡╤А╨╕╨░╨╗</h3>
                                    <p>теория по теме "${topic.title}" ╤Б╨║╨╛╤А╨╛ ╨▒╤Г╨┤╨╡╤В ╨┤╨╛╨▒╨░╨▓╨╗╨╡╨╜╨░</p>
                    ...
                    <div class="empty-state interactive">
                        ...
                        <div class="coming-soon-container">
                            ...
                            <div class="progress-placeholder">
                                ...
                                <span class="placeholder-text">Подготовка заданий...</span>
                            </div>
                        </div>
                        <div class="empty-actions">
                            <button class="notify-btn" onclick="this.innerHTML='<i class=\\"material-icons\\">notifications_active</i> ╨г╨▓╨╡╨┤╨╛╨╝╨╕╨╝'">
    ...
    renderPracticeCard(practice, difficulty) {
        ...
        return `
            <div class="practice-problem-card${isLocked ? ' locked' : ''}" 
                 onclick="${isLocked ? 'window.subscriptionManager.showPremiumLock(\'Сложный ╨╖╨░╨┤╨░╤З╨╕\')' : `window.subjectManager.startPractice('${practice.id}')`}">
                ${isLocked ? `
    ...
    switchPracticeDifficulty(difficulty) {
        ...
        if (difficulty === 'hard' && window.subscriptionManager && !window.subscriptionManager.isPremium) {
            window.subscriptionManager.showPremiumLock('Сложный ╨╖╨░╨┤╨░╤З╨╕');
            return;
        }
    ...
    performSearch() {
        ...
        const mockResults = [
            ...
            { type: 'practice', title: '╨а╨╡╤И╨╡╨╜╨╕╨╡ ╤Г╤А╨░╨▓╨╜╨╡╨╜╨╕╨╣ ╤Б ╨║╨╛╤А╨╜╤П╨╝╨╕', subject: 'math', grade: 8, description: 'Практические задания ╨┐╨╛ ╨║╨╛╤А╨╡╨╜╨╜╤Л╨╝ ╤Г╤А╨░╨▓╨╜╨╡╨╜╨╕╤П╨╝' },
            ...
            resultsContainer.innerHTML = filteredResults.map((result, index) => {
                ...
                <div class="search-result-item" onclick="window.subjectManager.selectSearchResult('${result.type}', '${result.title}')" style="--result-gradient: ${resultGradient};">
                    ...
                    <div class="search-result-content">
                        ...
                        <div class="search-result-meta">${subjectName} тАв ${result.grade} класс тАв ${result.type === 'title' ? '╨в╨╡╨╝╨░' : result.type === 'theory' ? '╨в╨╡╨╛╤А╨╕╤П' : '╨Я╤А╨░╨║╤В╨╕╨║╨░'}</div>
