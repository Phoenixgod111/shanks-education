...
class SubjectManager {
    ...
    getTopicIcon(topicTitle, subjectId) {
        ...
        // ╨Ь╨░╤В╨╡╨╝╨░╤В╨╕╨║╨░
        if (subjectId === 'math') {
            if (title.includes('числа') || title.includes('╨╕╤А╤А╨░╤Ж╨╕╨╛╨╜╨░╨╗╤М╨╜')) return 'numbers';
            if (title.includes('╨║╨╛╤А╨╜╨╕') || title.includes('╤А╨░╨┤╨╕╨║╨░╨╗')) return 'functions';
            if (title.includes('╤В╤А╨╕╨│╨╛╨╜╨╛╨╝╨╡╤В╤А') || title.includes('синус') || title.includes('косинус')) return 'timeline';
        ...
        if (subjectId === 'physics') {
            ...
            if (title.includes('╤Н╨╗╨╡╨║╤В╤А╨╕╤З') || title.includes('ток')) return 'bolt';
            if (title.includes('магнит')) return 'explore';
            if (title.includes('оптик') || title.includes('свет')) return 'visibility';
            if (title.includes('╤В╨╡╤А╨╝╨╛╨┤╨╕╨╜╨░╨╝╨╕╨║') || title.includes('тепло')) return 'whatshot';
            return 'science';
        }
        ...
        if (subjectId === 'biology') {
            ...
            if (title.includes('экосистем') || title.includes('╨┐╤А╨╕╤А╨╛╨┤')) return 'nature';
            if (title.includes('генетик')) return 'dna'; // ╨╕╨╗╨╕ ╨┤╤А╤Г╨│╨╛╨╣ ╨┐╨╛╨┤╤Е╨╛╨┤╤П╤Й╨╕╨╣
            return 'bug_report';
        }

        // ╨Р╨╜╨│╨╗╨╕╨╣╤Б╨║╨╕╨╣
        if (subjectId === 'english') {
            if (title.includes('глаго')) return 'sync';
            if (title.includes('существител')) return 'title';
    ...
    async init() {
        console.log('⚙️ SubjectManager: Starting initialization...');
    ...
    async showTopicsList(subjectId, grade) {
        ...
        console.log(`💾 Loading content for subject: ${subjectId}, grade: ${grade}`);
        await this.loadSubjectTopics(subjectId, grade);
        console.log(`💾 Content loaded, checking if topics exist...`);
                    ...
                    <div class="topics-list-compact">
                        ${topics && topics.length > 0 ? topics.map((topic, index) => {
                            ...
                            <div class="empty-state">
                                ...
                                <h3>╨в╨╡╨╝╤Л ╨╜╨╡ ╨╜╨░╨╣╨┤╨╡╨╜╤Л</h3>
                                <p>Темы для предмета "${subjectInfo?.name || subjectId}" в ${grade} классе ещё не добавлены</p>
                            </div>
                        `}
    ...
    goBackToSubjects() {
        ...
        console.log('⚠️ Navigation manager not available, using emergency fallback');
            ...
            <div id="lesson-screen" class="screen active">
                ...
                <div class="lesson-content">
                    ...
                    <div class="content-tab" id="quiz-tab">
                        <div class="tab-header">
                            <h2>тест╨╕╤А╨╛╨▓╨░╨╜╨╕╨╡ ╨╖╨╜╨░╨╜╨╕╨╣</h2>
                            <p class="tab-description">╨Я╤А╨╛╨▓╨╡╤А╤М╤В╨╡ ╤Б╨▓╨╛╨╕ ╨╖╨╜╨░╨╜╨╕╤П по теме</p>
                        </div>
                            ...
                            ${topic.quiz && topic.quiz.questions && topic.quiz.questions.length > 0 ?
                                ...
                                <div class="quiz-card" onclick="window.subjectManager.startQuiz('${topic.quiz.id || 'default'}')">
                                    <div class="quiz-header">
                                        ...
                                        <div class="quiz-info">
                                            <h3>тест по теме</h3>
                                            <p>${topic.quiz.description || `╨Я╤А╨╛╨▓╨╡╤А╤М╤В╨╡ ╤Б╨▓╨╛╨╕ ╨╖╨╜╨░╨╜╨╕╤П по теме "${topic.title}"`}</p>
                                        </div>
                                    </div>
                                    <div class="quiz-meta">
                                ...
                                <div class="empty-state interactive">
                                    ...
                                    <h3>тест по теме</h3>
                                    <p>тест по теме "${topic.title}" ╤Б╨║╨╛╤А╨╛ ╨▒╤Г╨┤╨╡╤В ╨┤╨╛╨▒╨░╨▓╨╗╨╡╨╜</p>
                                    <div class="coming-soon-container">
                                        <div class="coming-soon-badge pulse">╨б╨║╨╛╤А╨╛</div>
                                        <div class="progress-placeholder">
                                            <div class="progress-bar-placeholder">
            ...
            return `
                <div class="practice-content">
                    <div class="empty-state interactive">
                        ...
                        <h3>╨Я╤А╨░╨║╤В╨╕╤З╨╡╤Б╨║╨╕╨╡ ╨╖╨░╨┤╨░╨╜╨╕╤П</h3>
                        <p>╨г╨┐╤А╨░╨╢╨╜╨╡╨╜╨╕╤П по теме "${topic.title}" ╤Б╨║╨╛╤А╨╛ ╨▒╤Г╨┤╤Г╤В ╨┤╨╛╨▒╨░╨▓╨╗╨╡╨╜╤Л</p>
                        <div class="coming-soon-container">
                            <div class="coming-soon-badge pulse">╨б╨║╨╛╤А╨╛</div>
                            <div class="progress-placeholder">
                                <div class="progress-bar-placeholder">
    ...
    openLesson(lessonId) {
        ...
        if (window.app && window.app.showMessage) {
            window.app.showMessage('╨г╤А╨╛╨║ ╤Б╨║╨╛╤А╨╛ ╨▒╤Г╨┤╨╡╤В ╨╛╤В╨║╤А╤Л╤В! 📌');
        }
    }
    ...
    startPractice(practiceId) {
        ...
        if (window.app && window.app.showMessage) {
            window.app.showMessage('практика ╤Б╨║╨╛╤А╨╛ ╨▒╤Г╨┤╨╡╤В ╨┤╨╛╤Б╤В╤Г╨┐╨╜╨░! 👍');
        }
    }
    ...
    startQuiz(quizId) {
        ...
        if (window.app && window.app.showMessage) {
            window.app.showMessage('тест ╤Б╨║╨╛╤А╨╛ ╨▒╤Г╨┤╨╡╤В ╨┤╨╛╤Б╤В╤Г╨┐╨╡╨╜! ЁЯза');
        }
    }
    ...
    startQuiz(quizId) {
        // ╨Ы╨╛╨│╨╕╨║╨░ ╨┤╨╗╤П ╨╖╨░╨┐╤Г╤Б╨║╨░ ╤В╨╡╤Б╤В╨░
        app.showMessage('тест ╤Б╨║╨╛╤А╨╛ ╨▒╤Г╨┤╨╡╤В ╨┤╨╛╤Б╤В╤Г╨┐╨╡╨╜!');
    }
